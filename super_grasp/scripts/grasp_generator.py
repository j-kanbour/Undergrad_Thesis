...
from super_grasp.grasps import Grasps
from hsrb_interface import geometry
from super_grasp.superquadric import Superquadric
import sensor_msgs.point_cloud2 as pc2
from visualization_msgs.msg import Marker
from super_grasp.pointCloudData import PointCloudData
...
class GraspGenerator():
    def __init__(self): #need to add parameters here
        ...
        self.grasp_width = float(rospy.get_param("~grasp_width"))
        ...
        self.target_obeject_id = int(rospy.get_param("~target_object_id")) #if None, will process all detected objects
        self.target_obeject_class = rospy.get_param("~target_object_class") #if None, will process all detected objects
        self.orientation = rospy.get_param("~orientation") #Top or Front
        ...
        self.detection_sub = rospy.Subscriber(self.detections_topic, DetectionList, callback=self.main_callback)
    ...
    def main_callback(self, detection_msg):
        try:
            ...
            if self.run and detection_msg and detection_msg.objects:
                ...
                #loop through each detected object in frame
    ...
    def camera_to_base_transform(self, grasp_pose):
        ...
        try:
            ...
            # Compute direction from base to grasp point in XY plane
            ...
            forward_vector = R_horiz[:, 0]  # Y axis of rotation matrix
    ...
    def camera_to_hand_transform(self, grasp_pose):
        ...
        try:
            ...
            # This ignores the timestamp in grasp_pose and gets whatever is most recent
            T = self.tf_buffer.lookup_transform(
                target_frame, 
                grasp_pose.header.frame_id, 
                rospy.Time(0),  # Latest transform available
                rospy.Duration(2.0)
            )
            
            hand_pose = tf2_geometry_msgs.do_transform_pose(grasp_pose, T)
            
            # Optional: update the output timestamp to match the transform used
